...
const createCard = async (req, res) => {
  ...
  try {
    ...
    const [accountData] = await db.execute('SELECT Balance FROM Accounts WHERE AccountID = ?', [AccountID]);
    ...
    await db.execute('UPDATE Accounts SET Balance = Balance - 1500 WHERE AccountID = ?', [AccountID]);
...
const getCardByAccID = async (req, res) => {
  ...
  try {
    // execute to get the count of cards for the AccountID
    const [cardCountData] = await db.execute(
    ...
    // execute to get the card details from the Account_Cards table
...
const updateCardPIN = async (req, res) => {
  ...
  try {
    ...
    const [updateResult] = await db.execute(updateexecute, [NewPIN, AccountID, CardID, CardNumber]);
...
const deleteCard = async (req, res) => {
  ...
  try {
    ...
    const [deleteResult] = await db.execute(deleteexecute, [AccountID, CardNumber, CardType]);
...
const depositMoney = async (req, res) => {
  try {
    ...
    const [cardDetails] = await db.execute(
