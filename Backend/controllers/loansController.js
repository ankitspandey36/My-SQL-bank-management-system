...
const getLoans = async (req, res) => {
    try {
        ...
        const [data] = await db.execute("SELECT * FROM Loans");
...
const getLoansByCustomerID = async (req, res) => {
    try {
        ...
        if (!data || data.length === 0 || !data[0].LoanID) {
            ...
            const [customerCheck] = await db.execute('SELECT * FROM Customers WHERE CustomerID = ?', [customerid]);
...
const createLoan = async (req, res) => {
    try {
        ...
        if (!CustomerID || !LoanType) {
            ...
            const [customerAndAccount] = await db.execute(
      ...
      if(customerAndAccount[0].Balance<100000){
        ...
        await db.execute('UPDATE Accounts SET Balance = Balance + ? WHERE AccountID = ?', [loanAmount, AccountID]);
