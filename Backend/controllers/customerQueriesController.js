...
const createCustomerQuery = async (req, res) => {
    ...
    try {
        ...
        const [customerData] = await db.execute('SELECT * FROM Customers WHERE CustomerID = ?', [CustomerID]);
        ...
        const [availableEmployeeData] = await db.execute('SELECT EmployeeID FROM Employees LIMIT 1'); // Picking the first available employee (you can modify this logic)
        ...
        const [result]=await db.execute(insertQuery, [CustomerID, employeeID, queryDate, QueryType, Description, status]);
const [querydetails]=await db.execute(`select * from Customer_Queries where QueryID=?`,result.insertId)
...
const updateCustomerQuery = async (req, res) => {
    ...
    try {
        ...
        const [queryData] = await db.execute('SELECT * FROM Customer_Queries WHERE QueryID = ?', [QueryID]);
        ...
        const [updateResult] = await db.execute(updateQuery, [QueryType, Description, QueryID, CustomerID]);
