...
const getCustomerByID = async (req, res) => {
    try {
        ...
        const [data] = await db.execute(`SELECT c.*,a.AccountID,a.AccountType,a.Balance FROM Customers c
...
const updateCustomer = async (req, res) => {
    try {
        ...
        const [customerExists] = await db.execute("SELECT 1 FROM Customers WHERE CustomerID = ?", [CustomerID]);
...
const deleteCustomer = async (req, res) => {
    try {
        ...
        const result = await db.execute('DELETE FROM Customers WHERE CustomerID = ?', [CustomerID]);
