...
const createBranches=async(req,res)=>{
    try {
        ...
        const data=await db.execute('INSERT INTO Branches (BranchID, BranchName, Location, Phone) VALUES (?, ?, ?, ?)', [BranchID, BranchName, Location, Phone])
        if(!data){
            return res.status(404).send({
                success:false,
...
const updateBranch = async (req, res) => {
    try {
        ...
        const [checkResult] = await db.execute(checkQuery, [BranchID]);
