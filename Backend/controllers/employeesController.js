...
//         const [existingEmployee] = await db.execute('SELECT * FROM Employees WHERE EmployeeID = ?', [EmployeeID]);
...
const createEmployee = async (req, res) => {
    let connection;
    try {
        // 1. Add Email to the variables
        const { EmployeeID, FirstName, LastName, Position, Salary, HireDate, BranchID, Email } = req.body;


        console.log("--- RECEIVED BY BACKEND ---");
        console.log("BranchID received:", BranchID);
        console.log("Type of BranchID:", typeof BranchID);
        console.log("---");
        
        // Add Email to the validation check
        if (!EmployeeID || !FirstName || !LastName || !Position || !Salary || !HireDate || !BranchID || !Email) {
            return res.status(400).send({
                success: false,
                message: "All fields are required: EmployeeID, FirstName, LastName, Position, Salary, HireDate, BranchID, Email",
            });
        }
        ...
        const [existingEmployee] = await connection.execute('SELECT EmployeeID FROM Employees WHERE EmployeeID = ?', [EmployeeID]);
        if (existingEmployee.length > 0) {
            await connection.rollback();
            return res.status(400).send({
                success: false,
        ...
        // 2. Add Email to the INSERT statement and the values
        const sql = 'INSERT INTO Employees (EmployeeID, FirstName, LastName, Position, Salary, HireDate, BranchID, Email) VALUES (?, ?, ?, ?, ?, ?, ?, ?)';
        const values = [EmployeeID, FirstName, LastName, Position, Salary, HireDate, BranchID, Email];
