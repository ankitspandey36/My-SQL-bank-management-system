...
const getTransactionsByAccountID = async (req, res) => {
    try {
        ...
        const [data] = await db.execute('SELECT * FROM Transactions WHERE AccountID = ?', [accountID]);

        if (!data || data.length === 0) {
            // AccountID doesn't have any transactions, but let's also check if the AccountID exists in the Accounts table
            const [accountCheck] = await db.execute('SELECT * FROM Accounts WHERE AccountID = ?', [accountID]);
...
const createTransaction = async (req, res) => {
    ...
    try {
        ...
        await connection.execute('UPDATE Accounts SET Balance = ? WHERE AccountID = ?', [newBalance, AccountID]);
